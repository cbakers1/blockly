...
Blockly.PHP.scrub_ = function (a, b, c) {
    ...
    if (!a.outputConnection || !a.outputConnection.targetConnection) {
        ...
        e && (e = Blockly.utils.string.wrap(e, Blockly.PHP.COMMENT_WRAP - 3), d += Blockly.PHP.prefixLines(e, "// ") + "\n");
...
Blockly.PHP.controls_if = function (a) {
    ...
    if (a.getInput("ELSE") || Blockly.PHP.STATEMENT_SUFFIX) e =
        Blockly.PHP.statementToCode(a, "ELSE"), Blockly.PHP.STATEMENT_SUFFIX && (e = Blockly.PHP.prefixLines(Blockly.PHP.injectId(Blockly.PHP.STATEMENT_SUFFIX, a), Blockly.PHP.INDENT) + e), c += " else {\n" + e + "}";
    return c + "\n"
};
...
Blockly.PHP.math_number_property=function(a){var b=Blockly.PHP.valueToCode(a,"NUMBER_TO_CHECK",Blockly.PHP.ORDER_MODULUS)||"0",c=a.getFieldValue("PROPERTY");if("PRIME"==c)return[Blockly.PHP.provideFunction_("math_isPrime",["function "+Blockly.PHP.FUNCTION_NAME_PLACEHOLDER_+"($n) {","  // https://en.wikipedia.org/wiki/Primality_test#Naive_methods","  if ($n == 2 || $n == 3) {","    return true;","  }","  // False if n is NaN, negative, is 1, or not whole.","  // And false if n is divisible by 2 or 3.",
    "  if (!is_numeric($n) || $n <= 1 || $n % 1 != 0 || $n % 2 == 0 || $n % 3 == 0) {", "    return false;", "  }", "  // Check all the numbers of form 6k +/- 1, up to sqrt(n).", "  for ($x = 6; $x <= sqrt($n) + 1; $x += 6) {", "    if ($n % ($x - 1) == 0 || $n % ($x + 1) == 0) {", "      return false;", "    }", "  }", "  return true;", "}"]) + "(" + b + ")", Blockly.PHP.ORDER_FUNCTION_CALL];
    switch (c) {
        case "EVEN":
            var d = b + " % 2 == 0";
            break;
...
Blockly.PHP.procedures_defreturn=function(a){for(var b=[],c,d=a.workspace,e=Blockly.Variables.allUsedVarModels(d)||[],f=0;c=e[f];f++)c=c.name,-1==a.arguments_.indexOf(c)&&b.push(Blockly.PHP.variableDB_.getName(c,Blockly.Variables.NAME_TYPE));d=Blockly.Variables.allDeveloperVariables(d);for(f=0;f<d.length;f++)b.push(Blockly.PHP.variableDB_.getName(d[f],Blockly.Names.DEVELOPER_VARIABLE_TYPE));b=b.length?Blockly.PHP.INDENT+"global "+b.join(", ")+";\n":"";d=Blockly.PHP.variableDB_.getName(a.getFieldValue("NAME"),Blockly.Procedures.NAME_TYPE);
    ...
    var g = Blockly.PHP.statementToCode(a, "STACK"),
        h = Blockly.PHP.valueToCode(a, "RETURN", Blockly.PHP.ORDER_NONE) || "", k = "";
    g && h && (k = e);
...
Blockly.PHP.text_multiline = function (a) {
    return [Blockly.PHP.multiline_quote_(a.getFieldValue("TEXT")), Blockly.PHP.ORDER_ATOMIC]
};
...
"ADD"+c,Blockly.PHP.ORDER_COMMA)||"''";a="implode('', array("+b.join(",")+"))";return[a,Blockly.PHP.ORDER_FUNCTION_CALL]};Blockly.PHP.text_append=function(a){var b=Blockly.PHP.variableDB_.getName(a.getFieldValue("VAR"),Blockly.Variables.NAME_TYPE);a=Blockly.PHP.valueToCode(a,"TEXT",Blockly.PHP.ORDER_ASSIGNMENT)||"''";return b+" .= "+a+";\n"};
